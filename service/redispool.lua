...
function CMD.hincrby(uid, key, field, val)
   local db = getconn(uid) 
   local result = db:hincrby(key, field, val)
   return result
end
...
function CMD.redis_cmd(uid, cmd, ...)
	local db = getconn(uid)
	local result = assert(db[cmd], "not exist command "..cmd)(db, ...)
